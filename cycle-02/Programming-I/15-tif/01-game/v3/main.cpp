...
public:
    ...
    virtual void dibujarCalibracion(int x, int y, float gravedadActual) = 0; // Añadido parámetro gravedad
...
public:
    ...
    // --- CORRECCIÓN DE ALINEACIÓN DEL PANEL DE MICRÓFONO ---
    void dibujarCalibracion(int x, int y, float gravedadActual) override {
        int anchoPanel = 500;
...
int main() {
    ...
    float gravity = 0.4f; // Gravedad inicial
    ...
    while (!WindowShouldClose()) {
        ...
        // --- CONTROL GLOBAL DE GRAVEDAD (Usamos W/S para no interferir con el micro) ---
        if (IsKeyPressed(KEY_W)) gravity -= 0.05f;
        if (IsKeyPressed(KEY_S)) gravity += 0.05f;
        // Límites de seguridad para la gravedad
        ...
        switch (pantallaActual) {
            ...
            case JUEGO: {
                ...
                // Permitimos que baje hasta 625 antes de morir para que no se sienta injusto
        ...
        if (pantallaActual == MENU_CALIBRACION) {
            ...
            // MISTI DECORATIVO
            Vector2 p1 = { mistiMenu.x, 600 - mistiMenu.datos.altoVisualPx };
            Vector2 p2 = { mistiMenu.x - mistiMenu.datos.anchoBasePx/2, 600 };
            Vector2 p3 = { mistiMenu.x + mistiMenu.datos.anchoBasePx/2, 600 };
            DrawTriangle(p1, p2, p3, mistiMenu.datos.colorCuerpo);
            Vector2 n2 = Vector2Lerp(p1, p2, 0.25f); Vector2 n3 = Vector2Lerp(p1, p3, 0.25f);
            ...
            // PANELES DE CONTROL (Pasamos la gravedad actual para mostrarla)
            // Ajustamos la X a 250 para que el panel de microfono quede centrado
            ...
            // CRÉDITOS (Color corregido a BLACK)
            ...
            DrawText(TextFormat("Hitos Geologicos: %d", volcanesEsquivados), 20, 20, 25, DARKBLUE);
             // Mostrar gravedad actual en el juego también
