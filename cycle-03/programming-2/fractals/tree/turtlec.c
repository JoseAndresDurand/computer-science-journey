...
static bool turtleProcessEvents(Turtle *turtle) {
  ...
  return sfRenderWindow_isOpen(turtle->window) == 1;
}
...
TurtleApp *turtleAppCreate(unsigned int width, unsigned int height,
  ...
  sfVideoMode mode = {.size = {.x = width, .y = height}, .bitsPerPixel = 32};

  if (app == NULL)
    return NULL;

  app->window = sfRenderWindow_create(mode, title, sfResize | sfClose, 0, NULL);
  if (app->window == NULL) {
    free(app);
    return NULL;
  }
